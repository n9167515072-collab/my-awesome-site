...
- `live` is not available in this project (intentionally not vendored — see SKILL.md's Commands note); never suggest it. **The bundled `detect.mjs` is web-only.** If `setup.platform` is `ios`, `android`, or `adaptive`, don't lead with it; the HTML rule engine doesn't apply to native app code.
