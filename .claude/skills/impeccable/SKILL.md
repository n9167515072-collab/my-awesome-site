...
> **Note (this project):** the upstream `live` command (in-browser visual variant iteration — local preview server, DOM injection, framework adapters) is intentionally not vendored here. It's a large, separate subsystem with its own trust surface; everything else in this table works normally. See `.claude/skills/impeccable/VENDORING.md`.
